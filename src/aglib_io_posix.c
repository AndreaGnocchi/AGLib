...
bool read_all_file(int fd, sFileBuffer* out , sAllocator* a) {
  ...
  void* buf = (void*)ag_alloc(a, size + 1, false);
  if (!buf) return false;
...
bool dir_list (const char* path , sDirList* out, sAllocator* a) {
  ...
  if (!out->entries) { closedir(dir); return false; }
  ...
  while ((entry = readdir(dir))) {
    ...
    if (out->count >= capacity) {
      ...
      if (!newEntries) { closedir(dir); return false; }
      out->entries = newEntries;
    }

    size_t len     = strlen(entry->d_name);
    char* nameCopy = (char*)ag_alloc(a, len + 1, false);
    if (!nameCopy) { closedir(dir); return false; }
